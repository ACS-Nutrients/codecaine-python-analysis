...
```env
# PostgreSQL
DATABASE_URL=postgresql://<user>:<password>@<host>:5432/vitamin_analysis
...
└── services/
    ...
    ├── codef_service.py           # CODEF API 연동 (건강검진/처방기록)
    ├── agent_service.py           # AWS Bedrock Agent 호출
...
| 권한 | 용도 |
|------|------|
| `bedrock:InvokeAgent` | Bedrock Agent 호출 |
| `s3:GetObject`, `s3:PutObject` | CODEF 데이터 저장/조회 |
